...
Different levels of description of cPB. (a) At the finest level, PB is described through a united atom model. (b) At the moderately CG level, each monomer of PB is mapped onto one bead; the CG bead is located on the center-of-mass of the mono

VLM description: The figure is a plot of different levels of description of cPB. (a) At the finest level, PB is described through a united atom model. (b) At the moderately CG level, each monomer of PB is mapped onto one bead; the CG bead is located on the center-of-mass of the mono
...
At the mCG level, each monomer of cPB is mapped onto one coarse- grained bead. For mapping, one mCG bead is placed on the center- of- mass of each monomer of the atomistic chains (see Figure 1b). This model has the advantage of retaining structural features and therefore conserving the chemical identity of the atomistic model. To develop the mCG force field, it is assumed that the effective potential energy (in fact, free energy) of a mCG bead can be separated into bonded and pairwise nonbonded parts. Furthermore, the bonded interactions involve contributions from bonds, angles, and dihedral angles: \[U^{\mathrm{CG}} = U_{\mathrm{nonbonded}}^{\mathrm{CG}}(r) + U_{\mathrm{bond}}^{\mathrm{CG}}(l) + U_{\mathrm{angle}}^{\mathrm{CG}}(\theta) + U_{\mathrm{dihedral}}^{\mathrm{CG}}(\phi) \quad (1)\]
...
Here, \(r\) , \(l\) , \(\theta\) , and \(\phi\) are the distance between two mCG beads, mCG bond length, mCG bending angle, and mCG dihedral angle, respectively. Consistent with the above assumption, it is assumed that the probability distribution function of \(l\) , \(\theta\) , and \(\phi\) can be factorized into independent distribution functions: \[P(l,\theta ,\phi) = P(l)P(\theta)P(\phi) \quad (2)\]
...
Here, distributions of bond length and bending angle are normalized by taking into account their respective volume elements, \(l^{2}\) and \(\sin \theta\) (ignoring numerical prefactors). \(^{1}\) The mCG potentials are then modified in an iterative procedure through: \(^{9}\) \[U_{i + 1}^{\mathrm{CG}}(\xi) = U_{i}^{\mathrm{CG}}(\xi) + k_{\mathrm{B}}T\ln \frac{P_{i}(\xi)}{P_{\mathrm{target}}(\xi)} \quad (7)\]
...
The developed nonbonded and bonded mCG potentials together with the target atomistic local structural distributions and the distributions obtained from mCG simulations. (a) Nonbonded mCG pair potential and RDF (excluding the first three bonde

VLM description: The figure is a scientific plot that appears to be a visualization of a polymer melt simulation. The plot shows a nonbonded mCG pair potential and a radial distribution function (RDF) for a polymer melt. The nonbonded mCG pair potential is represented by a blue line, and the RDF is represented by a red line. The x-axis represents the distance between the beads, and the y-axis represents the probability density of the RDF. The plot also includes a legend that identifies the different types of data points. The data points are labeled with the variables and units, and the trends and patterns are visible in the plot.
...
(a) Mean-squared internal distances, analyzed at the level of monomers, for atomistic, mCG, SLSP models of 200-mer cPB. (b) Mean-squared end-to-end distance, \(\langle R_{ee}^{2}\rangle\) , and mean-squared radius of gyration, \(\langle R_{\m

VLM description: This plot is a comparative analysis of the mean-squared internal distances for different models of polymer melts. The x-axis represents the number of monomers in the polymer chain, ranging from 1 to 200. The y-axis represents the mean-squared internal distances, which are analyzed at the level of monomers. The data is presented for atomistic, mCG, and SLSP models of 200-mer cPB (copolymers of butadiene and styrene). The plot shows that the SLSP model results in shorter mean-squared internal distances compared to the atomistic and mCG models.
...
(a) MSD for chain center-of-mass \((g_{3}(t))\) and monomer center-of-mass \((g_{1}(t))\) for 100-mer chains of cPB at 413 K through atomistic and mCG simulations. (b) Orientational autocorrelation function, \(P(t)\) , of end-to-end vector an

VLM description: (a) The plot shows the mean square displacement (MSD) for the center-of-mass of a 100-mer chain of cPB at 413 K, as a function of time. The MSD is plotted for both the chain center-of-mass \(\left(g_{3}(t)\right)\) and the monomer center-of-mass \(\left(g_{1}(t)\right)\).

(b) The plot also shows the orientational autocorrelation function, \(P(t)\), of the end-to-end vector of the same 100-mer chain. The \(P(t)\) function is plotted as a function of time \(t\).

The plot appears to be a comparison of two different simulation methods: atomistic and molecular coarse-grained (mCG) simulations. The atomistic simulations are used to generate the data points, while the mCG simulations are used to generate the trends. The data points are plotted as a function
...
Chain-length dependence of time scaling factor, \(t_{\mathrm{mCG}}^{*}\) , calculated through matching of atomistic and mCG MSDs, at 413 K. Additionally, the chain length dependence of density, \(\rho\) , and (inset) \(t_{\mathrm{mCG}}^{*}\)

VLM description: The figure is a plot comparing the time scaling factor, \(t_{\mathrm{mCG}}^{*}\), calculated through matching of atomistic and mCG MSDs, with the chain length, at 413 K. The plot also shows the chain length dependence of density, \(\rho\), and an inset showing \(t_{\mathrm{mCG}}^{*}\). The data trends and patterns are not explicitly labeled, but the figure appears to be a scientific visualization of polymer melt rheology.
...
Distributions of bead-bead-bead angle \((P(\cos \theta))\) through median index (med) and center-of-mass (com) mapping schemes. The results are shown for cPB and the ideal chain. For cPB one bead represents 12 monomers and for the ideal chain

VLM description: This plot is a 2D histogram showing the distribution of bead-bead-bead angles \((\cos \theta)\) for two different polymer systems: cPB (coarse-grained polymer melts) and an ideal chain. The data is presented in two different mapping schemes: median index (med) and center-of-mass (com). The x-axis represents the angle \(\cos \theta\), while the y-axis represents the median index (med) or center-of-mass (com) of the beads. The y-axis is labeled with the number of beads, ranging from 1 to 12, corresponding to the number of monomers in the cPB system. The x-axis is labeled with the angle \(\cos \theta\), ranging from 0 to 1. The plot shows a clear distribution of angles for both systems, with a peak at around \(\cos \theta = 0.5\), indicating a preferred orientation of the beads in the
...
The nonbonded interaction of the model employed in our molecular dynamics simulation is an empirical pairwise potential. The potential has a two- fold purpose: (i) It suppresses density fluctuations and (ii) controls the repulsion of unlike bead types (in multicomponent systems). The potential is soft and quadratic, and it takes the form \[V_{\mathrm{nb}}(r) = A_{ij}\frac{k_{\mathrm{B}}T}{2}\left(1 - \frac{|r|}{\sigma}\right)^{2}\mathrm{for~}|r| < \sigma . \quad (13)\]
...
The \(N_{\mathrm{cg}}\) beads are bonded along the backbone by a bond- length potential that not only captures the Gaussian equilibrium statistics but, additionally, mimics the reduced extensibility for strong elongations and accounts for corrections to the limit of large degree of coarse- graining. \[V_{\mathrm{b}}(r) = \frac{k_{2}}{2} r^{2} + \frac{k_{4}}{2} r^{4}. \quad (14)\]
...
Distribution, \(p(z)\) , of the extension of 12-mer subchain in a polymer of 100 or 400 monomeric repeating units, obtained by simulations of the mCG model. For the highly coarse-grained SLSP model a 12-mer is represented by a single bead. Th

VLM description: The figure shows the distribution, \( p(z) \), of the extension of 12-mer subchains in a polymer of 100 or 400 monomeric repeating units, obtained by simulations of the mCG model. For the highly coarse-grained SLSP model, a 12-mer is represented by a single bead. The plot appears to be a histogram or a distribution plot, with the x-axis representing the extension of the 12-mer subchain and the y-axis representing the probability density function \( p(z) \). The data trends suggest that the distribution of subchain extensions is influenced by the number of monomeric repeating units in the polymer.
...
The SLSP model \(^{18}\) adds additional bonds to the system, which enforce spatial constraints similar to entanglements. As will become apparent later, the bond potential has to be of finite range, so we use the FENE potential \[V_{s s}(r) = -\frac{k_{s s}r_{s s}^{2}}{2}\log \left[1 - \left(\frac{r}{r_{s s}}\right)^{2}\right]. \quad (15)\]
...
For the two extreme descriptions of entanglement effects - the Rouse model \(^{68}\) and the tube model \(^{51}\) - there exist explicit predictions for \(P(t)\) . In the Rouse model, appropriate for unentangled, short polymer, \(P(t)\) adopts the form: \(^{51}\) \[P_{\mathrm{Rouse}}(t|\tau_{R}) = \sum_{p:\mathrm{odd}}\frac{8}{p^{2}\pi^{2}}\exp (-p^{2}t / \tau_{R}) \quad (17)\]
...
Timescale matching via the end-to-end vector correlation \(P(t)\) . The matching is obtained by horizontally shifting results of the SLSP model (on a logarithmic scale) until they overlay the mCG results at the "match point". In general, agre

VLM description: This plot is a time-scale matching analysis of polymer melts and coarsely-grained, and slip-spring models. The plot shows the end-to-end vector correlation \(P(t)\) for different models. The horizontal shift of the SLSP model results is used to match the mCG results on a logarithmic scale. The plot highlights the differences in the dynamics of the two models.
...
VLM description: This is a plot of a polymer melt rheology, which is a type of scientific figure from a research paper. The plot is a 3D surface plot, which means it shows the data in a three-dimensional space. The x-axis represents the temperature, the y-axis represents the strain rate, and the z-axis represents the stress. The data points are colored according to the stress, with different colors representing different stress levels. The plot shows a complex relationship between the temperature, strain rate, and stress, with some regions of high stress and low strain rate.
...
Timescale mapping \(t_{\mathrm{SS}}^{*}\) for the SLS

VLM description: The plot is a timescale mapping \(t_{\mathrm{SS}}^{*}\) for the SLS. The x-axis represents the time scale, and the y-axis represents the SLS value. The data points are plotted at specific times, with the SLS values increasing as the time scale increases. The plot shows a clear trend where the SLS value increases with time.
...
VLM description: The figure is a plot of a polymer chain, with the x-axis representing the position along the chain and the y-axis representing the force acting on the chain. The data points are connected by lines, indicating the force as a function of position. The plot shows a clear trend where the force decreases as the position along the chain increases.
...
Stress-autocorrelation of the non-diagonal stress tensor elements \(G(t)\) obtained by the mCG model and the SLSP model for the optimal SLSP fugacity. A vertical shift (on the logarithmic scale) is performed to match the the shear modulus \(

VLM description: The plot is a stress-autocorrelation diagram comparing the non-diagonal stress tensor elements \(G(t)\) obtained by the mCG model and the SLSP model for the optimal SLSP fugacity. The plot is on a logarithmic scale, with a vertical shift applied to match the shear modulus \(G(t)\). The axes are labeled with units of stress and time, respectively. The data trends show a comparison of the stress autocorrelation functions for the two models, highlighting any differences in their behavior.
...
Transferability between the mCG and SLSP model. The matching parameters are obtained with 400-mer cPB and transferred to simulations of a 200-mer melt without additional adjustments. This test demonstrates good agreement between the models w

VLM description: The plot is a 2D graph with a horizontal axis labeled "Temperature" and a vertical axis labeled "Entropy". The x-axis ranges from 0 to 100 degrees Celsius, while the y-axis ranges from 0 to 1000 Joules per mole. The graph shows a clear trend where entropy increases with temperature.
...
Molecular-weight dependence of the self-diffusion coefficient for bulk melts of cPB at 413 K. Depending on molecular weight, the results of the atomistic, mCG, or SLSP models are reported (for \(M = 10.8\) and \(21.6\mathrm{kg mol^{-1}}\) th

VLM description: The figure is a plot of the self-diffusion coefficient for bulk melts of cPB at 413 K, with molecular weight as a variable. The data is presented for two different molecular weights: 10.8 and 21.6 kg mol^-1. The plot shows the results of three different simulation models: atomistic, mCG, and SLSP. The x-axis represents the molecular weight, while the y-axis represents the self-diffusion coefficient. The data points for each model are plotted on the graph, with the molecular weight on the x-axis and the self-diffusion coefficient on the y-axis. The figure shows that the self-diffusion coefficient increases with increasing molecular weight for all three models.
...
Segmental mean-squared displacement, \(g_{1}(t)\) , normalized by \(\sqrt{t}\) for cPB chains of different lengths, obtained by simulations of: (a) the atomistic (dots), and mCG (lines) models and (b) the SLSP simulations. For a direct compa

VLM description: The figure shows the segmental mean-squared displacement, \(g_{1}(t)\), normalized by \(\sqrt{t}\), for cPB chains of different lengths, obtained by simulations of the atomistic (dots), mCG (lines), and SLSP models. The plot compares the results for (a) the atomistic and mCG models and (b) the SLSP simulations. The data trends and patterns are not explicitly labeled, but the figure appears to show that the SLSP simulations yield results that are closer to the atomistic and mCG models than the SLSP simulations.
...
The gradual crossover from unentangled to entangled polymer dynamics regime can also be observed through the calculation of the segmental MSD, \(g_{1}(t)\) , as a function of chain length. The Rouse model predicts the following scaling regimes for unentangled polymer chains (in the limit of very long chains): \(^{75}\) \[g_{1}(t) \propto \begin{cases} t^{\frac{1}{2}} & t < \tau_{\mathrm{R}} \\ t^{1} & t > \tau_{\mathrm{R}} \end{cases} \quad (18)\]
...
Chain-length dependence of shear-stress relaxation modulus, \(G(t)\) , for cPB at \(413\mathrm{K}\) from (a) mCG and (b) SLSP models. In panel (a), the experimental \(^{69}\) plateau modulus of \(1.4\) -PB at \(413\mathrm{K}\) is shown by a

VLM description: The figure is a plot comparing the shear-stress relaxation modulus, \(G(t)\), for cPB at \(413 \mathrm{~K}\) using two different models: mCG and SLSP. The experimental plateau modulus of \(1.4\)-PB at \(413 \mathrm{~K}\) is shown by a dashed line. The plot shows the chain-length dependence of \(G(t)\) for both models. The mCG model shows a higher modulus than the SLSP model. The figure also includes a caption that provides additional context about the experiment and the models used.
...
Calculated (through mCG and SLSP models) \(G^{\prime}(\omega)\) and \(G^{\prime \prime}(\omega)\) for 400-mer cPB ( \(M = 21.6\mathrm{kg}\) mol \(^{-1}\) ) together with the experimental data \(^{76}\) for a nearly monodisperse 1,4 rich PB (

VLM description: The plot is a frequency-dependent dielectric function \(G^{\prime}(\omega)\) and its derivative \(G^{\prime \prime}(\omega)\) for a 400-mer cPB (molecular weight \(M=21.6 \mathrm{~kg} \mathrm{~mol}^{-1}\) ) along with experimental data for a nearly monodisperse 1,4-rich PB. The plot shows a frequency-dependent dielectric function, with \(G^{\prime}(\omega)\) and \(G^{\prime \prime}(\omega)\) values increasing with frequency. The experimental data is represented by a solid line, while the calculated values are represented by a dashed line. The plot also includes a legend indicating the different data sets.
...
The complex modulus of oscillatory shear, \(G^{*}(\omega)\) , can be calculated from \(G(t)\) through: \[G^{*}(\omega) = G^{\prime}(\omega) + \mathrm{i}G^{\prime \prime}(\omega) = \mathrm{i}\omega \int_{0}^{\infty}\mathrm{e}^{-\mathrm{i}\omega t}G(t)\mathrm{d}t \quad (20)\]
...
where \(\omega\) is the angular frequency, and \(G^{\prime}(\omega)\) and \(G^{\prime \prime}(\omega)\) denote the storage and loss moduli. The above Laplace transform can be calculated by fitting \(G(t)\) to a series of Maxwell modes and analytic calculation of the integral: \[G(t) = \sum_{i = 0}^{n}g_{i}\mathrm{e}^{-t / \lambda_{i}} \quad (21)\]
...
Molecular-weight dependence of storage modulus, \(G^{\prime}(\omega)\) , and loss modulus, \(G^{\prime \prime}(\omega)\) calculated through the SLSP model for cPB at 413 K. For clarity we split the molecular weights to two panels. The longes

VLM description: This plot is a molecular-weight dependence of storage modulus, \(G^{\prime}(\omega)\), and loss modulus, \(G^{\prime \prime}(\omega)\), calculated through the SLSP model for cPB at 413 K. The plot is split into two panels, with the long-term data shown on the left and the short-term data on the right. The molecular weights are represented on the x-axis, with values ranging from 1000 to 1000000 Da. The y-axis represents the storage modulus, \(G^{\prime}(\omega)\), and the loss modulus, \(G^{\prime \prime}(\omega)\). The data trends show a general increase in both modulus components with increasing molecular weight. The short-term data shows a more pronounced increase in modulus compared to the long-term data.
...
The last part of our analysis concerns zero- shear viscosity, \(\eta_{0}\) , which can be calculated from \(G(t)\) as: \[\eta_{0} = \int_{0}^{\infty} \mathrm{d}t G(t) \quad (23)\]
...
Molecular-weight dependence of zero-shear viscosity for a bulk melt of cPB at 413 K. Depending of molecular weight, the results of the atomistic, mCG, and SLSP simulations are reported (for \(M = 10.8\) and \(21.6 \mathrm{kg} \mathrm{mol}^{-

VLM description: The figure is a plot of molecular-weight dependence of zero-shear viscosity for a bulk melt of cPB at 413 K. The plot shows the results of atomistic, mCG, and SLSP simulations for different molecular weights (M = 10.8 and 21.6 kg mol-1). The x-axis represents the molecular weight, while the y-axis represents the zero-shear viscosity. The data points for each simulation method are plotted on the graph, with the molecular weight on the x-axis and the viscosity on the y-axis. The figure shows that the zero-shear viscosity increases with increasing molecular weight for all three simulation methods.
...
Overview of the computational costs of the three different models in comparison (at 413 K).

VLM description: The plot is a bar chart comparing the computational costs of three different models at 413 K. The x-axis represents the model, with "SLSP" and "SLSP+CP" being the two main categories. The y-axis represents the computational cost in units of seconds. The bar heights indicate the computational costs for each model. The chart shows that the computational cost for the SLSP+CP model is significantly higher than the SLSP model, while the computational cost for the SLSP model is slightly higher than the SLSP+CP model.
...
Summary of the parameterization steps for the mCG and SLSP models. The mCG potentials are derived by matching bulk density and local structural distribution of the mCG chains to those of the atomistic chains. To match mCG and atomistic dynam

VLM description: The figure is a plot of the parameterization steps for the mCG and SLSP models. The mCG potentials are derived by matching bulk density and local structural distribution of the mCG chains to those of the atomistic chains. To match mCG and atomistic dynamics, the mCG time is scaled by a factor of 1000. The mCG interaction potentials are derived by matching the mCG model to those of the atomistic model through iterative atomistic dynamics. The mCG time is scaled by a factor of 1000, and the monomeric friction coefficient of the mCG model is 0.0001. Multiple Kuhn segments of a polymer chain are represented by a single bead, and the bonded interactions between beads do not prevent the polymer chains from stretching. The topological constraints are represented by the number of Kuhn segments in the polymer chains. A compensating pair potential is used to preserve the molecular properties unaltered upon the introduction
...
Comparison of the \(G(t)\) calculated as chain average versus the calculation via the total stress with a 800mer chain. The chain average offers the better statistics, but has a slightly softer decay. The important longest relaxation time is

VLM description: The figure compares the calculated \(G(t)\) values for a 800mer chain using a chain average method versus the total stress method. The chain average method provides better statistics but has a slightly softer decay. The longest relaxation time is highlighted.
...
Mean-squared displacement of monomers, \(g_{1}(t)\) , normalized by \(t^{0.5}\) for (a) 10-mer, (b) 30-mer, and (c) 50-mer cPB chains from atomistic and mCG simulations. The predicted \(g_{1}(t)\) curves of the Rouse model for chains contain

VLM description: The figure shows the mean-squared displacement of monomers, \(g_{1}(t)\), normalized by \(t^{0.5}\), for three different chain lengths (10-mer, 30-mer, and 50-mer) of cPB chains from atomistic and mCG simulations. The predicted \(g_{1}(t)\) curves of the Rouse model for chains contain a peak at \(t=0\), which represents the initial displacement of the monomers. The figure also shows that the \(g_{1}(t)\) curves for the atomistic simulations are closer to the Rouse model predictions than those for the mCG simulations.
